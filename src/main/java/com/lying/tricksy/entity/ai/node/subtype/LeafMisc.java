		...
		return new INodeTickHandler<LeafNode>()
				{
					...
					{
						if(tricksy.getType() == TFEntityTypes.TRICKSY_GOAT && 
								!tricksy.hasVehicle() && tricksy.isOnGround() && !tricksy.isTouchingWater() && !tricksy.isInLava() && !tricksy.getWorld().getBlockState(tricksy.getBlockPos()).isOf(Blocks.HONEY_BLOCK))
						{
							BlockPos target = getOrDefault(CommonVariables.VAR_POS, parent, local, global).as(TFObjType.BLOCK).get();
							if(target.getY() - tricksy.getBlockPos().getY() > 10)
								return Result.FAILURE;
							...
							// FIXME Implement proper velocity calculation
							tricksy.addVelocity(0D, 1D, 0D);
							
							if(parent.isRunning() && tricksy.isOnGround())
							{
								tricksy.setTreePose(tricksy.defaultPose());
